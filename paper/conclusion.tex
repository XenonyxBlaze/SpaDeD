...
Second, fine-grained spatial attention is highly effective when regularized appropriately. While global pooling dilutes subtle signals, unconstrained multi-head attention tends to collapse onto redundant regions. Integrating the Regional Independence Loss ($\mathcal{L}_{RIL}$) with AGDA forces attention heads to distribute across independent facial zones, which yields substantial performance gains on high-fidelity forgeries that lack obvious global anomalies.
