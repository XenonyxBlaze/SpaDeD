...
\begin{itemize}
    ...
    \item Because the spatial descriptors $x_t$ are invariant across time ($x_t = x$), the BiLSTM recurrent state rapidly reaches a fixed-point embedding ($h_T \approx \phi(x)$). 
...
\textbf{Bilinear Attention Pooling (BAP):} Let $\mathbf{F} \in \mathbb{R}^{C \times (HW)}$ denote the unrolled feature matrix and $\mathbf{A} = [A_1; A_2; \dots; A_M] \in \mathbb{R}^{M \times (HW)}$ denote the stacked attention map matrix. Bilinear Attention Pooling computes the outer-product aggregation:
\begin{equation}
...
\end{equation}
Each regional vector is $L_2$-normalized: $V_k \leftarrow \frac{V_k}{\|V_k\|_2 + \epsilon}$, yielding a parts-based descriptor where each head isolates forensic evidence from specific facial regions \citep{Zhao2021MultiAttentional}.
...
\begin{enumerate}
    \item \textbf{Multi-Head Feature Concatenation:} The $M=4$ normalized regional vectors $\{V_1^{(t)}, \dots, V_M^{(t)}\}$ are concatenated:
    \begin{equation}
        V_{concat}^{(t)} = \big[V_1^{(t)} \,\|\, V_2^{(t)} \,\|\, V_3^{(t)} \,\|\, V_4^{(t)}\big] \in \mathbb{R}^{M \cdot C} = \mathbb{R}^{9216}.
    \end{equation}
...
\begin{table}[ht]
    ...
    \begin{tabular}{l|l|l|l}
        ...
        BAP Pooling & $F$ and $A$ & $(B \cdot T, 4, 2304)$ & $M=4$ vectors $V_k \in \mathbb{R}^{2304}$ \\
...
Unconstrained multi-head attention is susceptible to attention collapse, where all heads co-activate on the single most salient visual artifact (such as mouth or eye regions). To enforce spatial diversity and representation orthogonality, the network is trained with a composite objective combining Cross-Entropy Classification Loss ($\mathcal{L}_{CE}$) with a dual-regularized Regional Independence Loss ($\mathcal{L}_{RIL}$).

\paragraph{Regional Independence Loss (\texorpdfstring{$\mathcal{L}_{RIL}$}{L\_RIL})}
The Regional Independence Loss is formulated with two complementary objectives:
\begin{equation}
    \mathcal{L}_{RIL} = \mathcal{L}_{spatial} + \gamma \, \mathcal{L}_{feat},
\end{equation}
where $\gamma = 1.0$ balances spatial separation and feature decorrelation.

1. \textbf{Spatial Attention Map Non-Overlap Loss ($\mathcal{L}_{spatial}$):} Directly penalizes spatial co-activation between 2D attention maps $A_i$ and $A_j$ across spatial coordinates $(x,y)$, normalized over all $\binom{M}{2}$ head pairs:
\begin{equation}
    \mathcal{L}_{spatial} = \frac{2}{M(M-1)} \sum_{i=1}^{M-1} \sum_{j=i+1}^{M} \sum_{x=1}^{H} \sum_{y=1}^{W} A_i(x,y) \cdot A_j(x,y).
\end{equation}
Because spatial attention weights sum to 1 ($\sum_{x,y} A_k(x,y) = 1$), minimizing $\mathcal{L}_{spatial}$ drives the overlap integral toward zero, explicitly forcing each attention head to cover physically disjoint facial zones (e.g., eyes, nose, mouth, blending contour).

2. \textbf{Regional Feature Orthogonality Loss ($\mathcal{L}_{feat}$):} Since the regional vectors $V_k$ are $L_2$-normalized, their cosine similarity reduces to inner product $V_i^T V_j$. The feature orthogonality loss penalizes redundant semantic representations:
\begin{equation}
    \mathcal{L}_{feat} = \frac{2}{M(M-1)} \sum_{i=1}^{M-1} \sum_{j=i+1}^{M} \max\big(0,\, V_i^T V_j - m\big),
\end{equation}
where $m = 0.2$ is a margin hyperparameter.
...
\end{equation}
where $\mathcal{L}_{CE}$ is standard cross-entropy loss and $\lambda = 0.5$ is the weighting coefficient determined via sensitivity sweep (Section \ref{sec:sensitivity}).
