...
\paragraph{Manuscript Changes:} Section 3.4.1 (Equations 8--11) and the ablation analysis in Section 4.3 (Table 6) have been thoroughly updated.
...
\noindent\textbf{Author Response:} We appreciate the Reviewer's request for formal clarity. We have revised \textbf{Section 3.3.3 (Equations 3--6)} to provide the explicit matrix algebraic definition of Bilinear Attention Pooling and clarify the precise order of tensor operations.
...
\paragraph{Manuscript Changes:} Section 3.3.3 (Equations 3--6) and Figure 1 have been revised with the complete matrix formulation and dimensionality flow.
...
\noindent\textbf{Author Response:} We have expanded \textbf{Section 3.3.4 (Equations 7--9)} and added \textbf{Table 1 (Tensor Dimension Trace Table)} to define every dimensional transition across the entire network:
...
\begin{table}[ht]
\centering
\caption{Full Tensor Dimension Trace across the Proposed Architecture}
...
\paragraph{Manuscript Changes:} Section 3.3.4, Equations 7--9, and Table 1 provide the full mathematical and tensor derivation.
...
\begin{itemize}
    ...
    \item \textbf{Reported Metrics:} In Table 4 (Section 4.4.1), each cell reports the $\text{Mean} \pm \text{Standard Deviation}$ across all 5 evaluation checkpoints.
\end{itemize}

\paragraph{Manuscript Changes:} Section 3.2.3 and Section 4.4.1 (Table 4) now explicitly define this protocol.
...
\begin{enumerate}
    \item \textbf{Mathematical Frame Replication \& Invariant Fixed-Point Convergence:}  
    ...
    \begin{itemize}
        ...
        \item \textbf{Video Categories (FS, FR):} The \textbf{BiLSTM sequence module} delivers an essential complementary boost ($+2.58\%$ AUC on Celeb-DF, Table 2) by capturing inter-frame jitter, boundary flickering, and irregular blinking dynamics.
    \end{itemize}
\end{enumerate}

\paragraph{Manuscript Changes:} Section 3.2.4 and Section 4.4.2 explicitly document this modality separation and fixed-point analysis.
...
\noindent\textbf{Author Response:} We thank the Reviewer for emphasizing statistical rigor. We have added \textbf{Section 4.4.3 and Table 5}, conducting two-tailed paired $t$-tests across the 5 cross-validation folds ($df = 4$) with family-wise error rate (FWER) and false discovery rate (FDR) corrections:
...
\begin{enumerate}
    ...
    \item \textbf{Statistical Significance Table (Table 5):}
\end{enumerate}
...
\midrule
\textbf{Train FS $\to$ Test FS} & $+1.1\%$ & $4.568$ & $0.0103$ & $0.0824$ & Significant ($q = 0.0103$) \\
\textbf{Train FS $\to$ Test FR} & $+4.2\%$ & $11.611$ & $< 0.0001$ & $< 0.0008$ & \textbf{Significant} ($p < \alpha_{adj}$) \\
\textbf{Train FS $\to$ Test EFS} & $+3.4\%$ & $7.694$ & $0.0015$ & $0.0120$ & \textbf{Significant} ($p < \alpha_{adj}$) \\
\textbf{Train FS $\to$ Test FE} & $+2.7\%$ & $8.409$ & $0.0011$ & $0.0088$ & \textbf{Significant} ($p < \alpha_{adj}$) \\
\textbf{Train FR $\to$ Test FS} & $+7.7\%$ & $17.424$ & $< 0.0001$ & $< 0.0008$ & \textbf{Significant} ($p < \alpha_{adj}$) \\
\textbf{Train FR $\to$ Test FR} & $+2.1\%$ & $7.457$ & $0.0017$ & $0.0136$ & \textbf{Significant} ($p < \alpha_{adj}$) \\
\textbf{Train FR $\to$ Test EFS} & $+2.3\%$ & $6.358$ & $0.0031$ & $0.0248$ & \textbf{Significant} ($p < \alpha_{adj}$) \\
\textbf{Train FR $\to$ Test FE} & $+5.1\%$ & $14.099$ & $< 0.0001$ & $< 0.0008$ & \textbf{Significant} ($p < \alpha_{adj}$) \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{Non-Parametric Confirmation:} Wilcoxon signed-rank tests yielded $W=0, p=0.03125$.
        \item \textbf{Conclusion:} All 8 cross-forgery conditions reject the null hypothesis under Benjamini-Hochberg FDR control ($q < 0.05$), and 6 of 8 remain significant under the strict Bonferroni threshold ($\alpha_{adj} = 0.00625$).
    \end{itemize}
\end{enumerate}

\paragraph{Manuscript Changes:} Section 4.4.3 and Table 5 document the full statistical analysis.
