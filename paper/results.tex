...
Without $\mathcal{L}_{RIL}$, all four attention heads collapse onto the mouth or eye regions. Enforcing the dual spatial non-overlap ($\mathcal{L}_{spatial}$) and feature orthogonality ($\mathcal{L}_{feat}$) constraints forces the attention heads to distribute across mutually distinct facial zones (left eye, right eye, nose/mouth, and blending perimeter). Pairing $\mathcal{L}_{RIL}$ with soft AGDA achieves peak performance ($97.60\%$ ACC and $67.44\%$ AUC).
